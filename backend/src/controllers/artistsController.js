...
const addArtistWithProfile = async (req, res) => {
    try {
        const { role, firstName, middleName, lastName, username, email, password, bio, phoneNumber, gender, birthdate, age } = req.body
        ...
        res.status(201).json({ message: "Artist account created successfully", data: { newArtist, newArtistProfile } })
    } catch (err) {
        console.error("Error in creating artist account:", err)
        res.status(500).json({ err: "Failed to create artist account. Please try again later." })
    }
}
...
const updateArtist = async (req, res) => {
    try {
        ...
        const { firstName, middleName, lastName, username, email, password, bio, phoneNumber, gender, birthdate, age } = req.body
        ...
        res.status(201).json({ message: "Artist account updated successfully", data: { artistProfile } })
    } catch (error) {
...
const loginArtist = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Login successful", newtoken: token, data: artist})
    } catch (error) {
